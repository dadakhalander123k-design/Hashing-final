    ...
    <div className="w-full max-w-2xl mx-auto font-sans">
      <div className="bg-white dark:bg-[#111827] border border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-sm dark:shadow-[0_8px_30px_rgba(0,0,0,0.35)] overflow-hidden transition-all duration-200">
        ...
        <div className="px-5 py-3.5 bg-slate-50/80 dark:bg-[#0B1120] border-b border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
          ...
          <span className="bg-white dark:bg-[#111827] px-2.5 py-1 rounded-lg border border-blue-100 dark:border-slate-700 font-mono font-bold text-xs text-[#2563EB] dark:text-[#3B82F6] shadow-2xs">
        ...
        <div className="p-5 sm:p-7 flex flex-col items-center gap-5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 w-full">
            ...
            >
              ...
              <span className={`text-[10px] uppercase tracking-widest font-bold font-mono ${isCalculated ? 'text-blue-200 dark:text-blue-100' : 'text-slate-400 dark:text-slate-500'}`}>
                Incoming Key
              </span>
              ...
              {isCalculated && (
                <span className="text-[10px] font-bold bg-white/20 dark:bg-[#1D4ED8] text-white px-2 py-0.5 mt-1 rounded-md uppercase font-mono tracking-wider backdrop-blur-xs">
                  Drag to slot
                </span>
              )}
            </div>
              ...
              {!isCalculated ? (
                <>
                  <div className="flex items-center gap-2">
                    <span className="text-xs uppercase font-bold tracking-widest text-[#2563EB] dark:text-[#3B82F6] bg-blue-50 dark:bg-blue-950/60 px-2.5 py-1 rounded-lg border border-blue-100 dark:border-blue-500/30 font-mono shadow-2xs">
                  ...
                  <form onSubmit={handleManualSubmit} className="flex flex-wrap items-center gap-2 mt-1 w-full">
                    <input
                      ...
                      className="w-28 px-3 py-2 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white font-mono text-center font-bold text-base focus:border-[#2563EB] dark:focus:border-[#3B82F6] focus:outline-hidden shadow-2xs"
                    />
                    <button
                ...
                <>
                  ...
                  <div className="text-xl sm:text-2xl font-mono font-bold text-slate-900 dark:text-white flex items-center flex-wrap gap-2">
                    ...
                    <span className="px-3.5 py-1 bg-[#2563EB] dark:bg-[#2563EB] text-white font-bold text-lg rounded-xl shadow-xs dark:shadow-none border border-[#1D4ED8]">
                      Index [{calculatedIndex < 10 ? `0${calculatedIndex}` : calculatedIndex}]
                    </span>
                  </div>

                  {isProbing && probeFormulaStr && (
                    <div className="bg-blue-50/80 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-500/30 rounded-xl p-3 w-full text-xs text-slate-900 dark:text-slate-200 shadow-2xs">
